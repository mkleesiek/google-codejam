...
TEST( Main, Randomized )
{
	for (size_t i = 0; i < 100; i++) {
		...
		ostringstream expectation;

		const size_t T = 10;

		const uint8_t U = 16;
		const uint64_t Q_max = powi<uint64_t>(10, U)-1;
		...
		for(size_t t = 0; t < T; t++) {
			...
			for (size_t d = 0; d < 10; d++) {
				string new_digit;
				do {
					new_digit = random_string(1);
				}
				while (D.find(new_digit.front()) != string::npos);
				D[d] = new_digit.front();
			}

			input << U << endl;
			for (uint16_t i = 0; i < 10000; i++) {
				uint64_t M = uniform_int_distribution<uint64_t>{1, Q_max}(mt);
				uint64_t N = uniform_int_distribution<uint64_t>{1, M}(mt);
